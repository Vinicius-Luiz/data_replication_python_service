...
class MetadataConnectionManager:
    ...
    APPLY_EXCEPTION_REQUIRED_COLUMNS = [
        "schema_name",
        "table_name",
        "message",
        "type",
    ...
    def insert_stats_cdc(self, data: Dict, **kargs) -> None:
        ...
        try:
            ...
            e = InsertMetadataError(f"Erro ao inserir dados na tabela stats_cdc: {e}")
            logger.critical(e)
    ...
    def insert_dlx_message(self, data: Dict, **kargs) -> None:
        """
        ...
        try:
            ...
            if not all(key in data for key in self.DLX_MESSAGE_REQUIRED_COLUMNS):
                e = RequiredColumnsError(
                    f"Data must contain all required keys: {self.DLX_MESSAGE_REQUIRED_COLUMNS}"
                )
                logger.critical(e)

            cursor = self.connection.cursor()
            cursor.execute(
                Query.SQL_INSERT_DLX_MESSAGE,
                (
                    data["task_name"],
                    data["transaction_id"],
                    data["message_id"],
            ...
            logger.critical(f"Erro ao inserir dados na tabela dlx_message: {e}")

    def insert_apply_exceptions(self, data: Dict, **kargs) -> None:
        """
        ...
        try:
            ...
            if not all(key in data for key in self.APPLY_EXCEPTION_REQUIRED_COLUMNS):
                e = RequiredColumnsError(
                    f"Data must contain all required keys: {self.APPLY_EXCEPTION_REQUIRED_COLUMNS}"
                )
                logger.critical(e)

            cursor = self.connection.cursor()
            cursor.execute(
                Query.SQL_INSERT_APPLY_EXCEPTIONS,
                (
                    data["schema_name"],
                    data["table_name"],
                    data["message"],
            ...
            logger.critical(f"Erro ao inserir dados na tabela apply_exceptions: {e}")
